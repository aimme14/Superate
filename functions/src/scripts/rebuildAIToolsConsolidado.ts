/**
 * Reconstruye la caché de lectura AI_Tools/consolidado_* a partir de todos los
 * documentos de herramientas (excluye IDs que empiezan por consolidado_).
 *
 ...
 *
 * Orden de `items`: igual que aiToolsService.getAll() — nombre (es), luego
 * createdAt descendente. Cada elemento es solo `data()` del documento (sin id).
 *
 * Uso (desde la carpeta functions):
 *   npm run rebuild-ai-tools-consolidado
 *
  ...
  for (const item of items) {
    ...
    if (current.length > 0) {
      ...
      if (sizeSingleNew > MAX_CONSOLIDATED_DOC_BYTES) {
        ...
        throw new Error(
          `Una sola herramienta supera el tamaño máximo permitido para un documento (~${approx} KiB JSON). Reduce el contenido o divide el dato manualmente.`
        );
      }
      current.push(item);
    } else {
...
/**
 * Mismo criterio que `aiToolsService.getAll()`: nombre (es), luego createdAt más reciente primero.
 */
function sortSourceDocsLikeGetAll(
  docs: QueryDocumentSnapshot[]
): QueryDocumentSnapshot[] {
  return [...docs].sort((a, b) => {
    const da = a.data();
    const db = b.data();
    const nameA = String(da.nombre ?? '');
    const nameB = String(db.nombre ?? '');
    const byName = nameA.localeCompare(nameB, 'es');
    if (byName !== 0) return byName;
    return createdAtMillis(db) - createdAtMillis(da);
  });
}

async function rebuildAIToolsConsolidado(): Promise<void> {
  const db = getStudentDatabase();
  const col = db.collection('AI_Tools');

  const snapshot = await col.get();
  const sourceDocs = sortSourceDocsLikeGetAll(
    snapshot.docs.filter((d) => !d.id.startsWith(CONSOLIDADO_ID_PREFIX))
  );
  ...
  console.log(
    `✅ AI_Tools consolidado: ${totalItems} herramientas en ${totalParts} documento(s) (${docIdsToWrite.join(', ')}).`
  );
}

async function main(): Promise<void> {
  await rebuildAIToolsConsolidado();
}
