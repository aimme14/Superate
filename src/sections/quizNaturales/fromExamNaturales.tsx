...
const ExamWithFirebase = () => {
    ...
    return (
      ...
      >
        ...
        <div className="w-full lg:w-56 flex-shrink-0 relative z-10">
          <div className={`${theme.cardBackground} border rounded-lg p-2.5 sticky top-4 shadow-lg backdrop-blur-sm`}>
            ...
            <div className="grid grid-cols-6 gap-1.5 mb-3">
              {examData.questions.map((q, index) => {
                ...
                const isReachable = index <= maxReachedQuestion + 1; // Preguntas alcanzadas + siguiente disponible
                const isLocked = index > maxReachedQuestion + 1; // Preguntas futuras bloqueadas
                ...
                return (
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      if (!isLocked && isReachable) {
                        changeQuestion(index);
                      }
                    }}
                    disabled={isLocked}
                    className={`relative h-7 w-7 rounded-md flex items-center justify-center text-[10px] font-bold transition-all duration-200 ${
                      isLocked 
  ...
  const mainTheme = getQuizTheme('naturales')
  return (
    <div 
      className="min-h-screen quiz-gradient-bg relative"
      style={getQuizBackgroundStyle(mainTheme)}
    >
