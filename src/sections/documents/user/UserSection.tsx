...
interface UserSectionProps extends ThemeContextProps { id: string | undefined }

const UserSection = ({ id, theme }: UserSectionProps) => {
    ...
    <main className="container p-2 sm:p-4">
      <Tabs value={tab} onValueChange={handle}>
        ...
        <TabsContent value="table">
          <TableUserSection theme={theme} onChange={() => handle('form')} params={params} />
        </TabsContent>
        <TabsContent value="form">
          <Card className={cn('relative w-full', theme === 'dark' ? 'bg-zinc-800' : 'bg-white')}>
            <FormUserSection id={id} theme={theme} onChange={() => handle('table')} />
          </Card>
        </TabsContent>
      </Tabs>
    </main>
