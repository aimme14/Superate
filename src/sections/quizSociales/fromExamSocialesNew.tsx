import { Clock, ChevronRight, Send, Brain, AlertCircle, CheckCircle2, BookMarked, Timer, HelpCircle, Users, Play, Maximize, X, Database, ZoomIn, Shield } from "lucide-react"
...
// Configuración del examen de Ciencias Sociales
const examConfig = {
  subject: "Ciencias Sociales",
  phase: "first" as const,
  examId: "exam_sociales_001", // ID único del examen
  title: "Examen de Ciencias Sociales",
  description: "Evaluación de conocimientos en historia, geografía y cívica",
...
const ExamWithFirebase = () => {
  ...
  useEffect(() => {
    ...
    const loadQuiz = async () => {
      console.log('=== INICIANDO CARGA DEL CUESTIONARIO DE CIENCIAS SOCIALES ===');
  ...
  const saveToFirebase = async (timeExpired = false, lockedByTabChange = false) => {
    if (!quizData || !userId) {
      console.error('❌ No se puede guardar: falta quizData o userId', { quizData: !!quizData, userId: !!userId });
      return;
    }

    console.log('💾 Iniciando guardado de respuestas en Firebase...');
    ...
    try {
      ...
      console.error('❌ Error guardando examen en Firebase:', error)
    ...
    const handleVisibilityChange = () => {
      if (!examLocked) {
        if (document.hidden) {
          ...
          if (tabChangeCount === 1 && !showTabChangeWarning) {
            setShowTabChangeWarning(true);
          }
        }
      }
      ...
      >
        ...
        <div className="w-full lg:w-56 flex-shrink-0 relative z-10">
          <div className={cn(`${theme.cardBackground} border rounded-lg p-2.5 sticky top-4 shadow-lg backdrop-blur-sm`, appTheme === 'dark' ? 'bg-zinc-800 border-zinc-700' : '')}>
            <h3 className={cn("text-xs font-semibold mb-2 uppercase tracking-wide", appTheme === 'dark' ? 'text-gray-300' : 'text-gray-700')}>
              Navegación
            </h3>
